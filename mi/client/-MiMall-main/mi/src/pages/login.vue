<template>
  <div>
    ...
    <div class="center">
      ...
      <!-- center-login -->
      <div class="center-login" v-show="changeregister == false">
        <div
          :class="{
            login: true,
        ...
        >
          <div class="login-header" @click="clicklogin">
            <span>登录</span>
            <span class="register-header" @click.stop="clickregister"
              >注册</span
            >
          </div>
          <div
      ...
      <!-- center-register -->
      <div class="center-login" v-show="changeregister == true">
        <div
          :class="{
            login: true,
            login1: click1 + click2 == 1,
        ...
        >
          <div class="login-header1" @click="clicklogin">
            <span>登录</span>
            <span class="register-header" @click.stop="clickregister"
              >注册</span
            >
          ...
          >
            ...
            >
            <!-- 注册的用户名或密码格式不正确 -->
            <span v-if="userInfo.length == 0"> </span>
          </div>
          <div :class="{ notification: true, notification_display: display }">
            <i class="iconfont icon-gantanhao"></i>请您同意用户条款
          </div>
...
export default {
  ...
  methods: {
      ...
      if (this.checked) {
        ...
        this.axios({
          ...
          if (this.userInfo.length == 1) {
            ...
            // 用户不存在则继续写入数据到数据库
            var url = `${this.CURL}users/register?username=${this.user_name}&userpwd=${this.user_password}`;
            this.axios({
              method: "get",
              url,
            }).then((response) => {
    ...
    clicklogin() {
      // console.log(this.changeregister);
      this.changeregister = false;
    },
    clickregister() {
      this.changeregister = true;
      // console.log(this.changeregister);
    },
  },
};
